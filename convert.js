...
- doggyDns: 恢复狗狗机场节点所需的 DNS 策略（默认 false）
...
(() => {
  ...
  function buildDoggyGoNameserverPolicy(proxies) {
    for (const proxy of proxies) {
      const server = proxy.server?.toLowerCase();
      const password = proxy.password;
      if (!server || typeof password !== "string" || password.length === 0 || !DOGGYGO_SERVER_DOMAINS.some(
  ...
  function buildDns({
    ...
    const defaults = fakeIPEnabled ? buildDnsConfig({
      mode: "fake-ip",
      ipv6Enabled,
    ...
    if (doggyDnsEnabled && source?.["nameserver-policy"] === void 0) {
  ...
  var FAKE_IP_FILTER, snifferConfig, CN_NAMESERVERS, OVERSEAS_NAMESERVERS, DOGGYGO_SERVER_DOMAINS;
  var init_dns = __esm({
    "src/dns.ts"() {
      "use strict";
      FAKE_IP_FILTER = [
      ...
      CN_NAMESERVERS = ["223.5.5.5", "119.29.29.29", "180.184.1.1"];
      OVERSEAS_NAMESERVERS = [
        "quic://dns0.eu",
      ...
      DOGGYGO_SERVER_DOMAINS = ["quandao.com", "jiandaoyun.com"];
    }
  });
